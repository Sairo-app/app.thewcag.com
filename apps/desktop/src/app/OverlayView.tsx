...
export function OverlayView() {
  ...
  useEffect(() => {
    ...
    void desktop.invoke<OverlaySession>("overlay:ready").then(apply).catch(() => undefined);
    return stop;
  }, []);
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "Escape") void desktop.invoke("overlay:cancel");
      ...
      if (event.key === "Enter" && session?.mode !== "capture" && session?.mode !== "measure") void pick(cursor);
    };
  ...
  return <div className={`overlay-view ${regionMode ? "region-mode" : "picker-mode"}`} onMouseMove={(event) => setCursor({ x: event.clientX, y: event.clientY })} onMouseDown={(event) => { if (regionMode && event.button === 0) { setStart({ x: event.clientX, y: event.clientY }); setDragging(true); } }} onMouseUp={(event) => { if (regionMode && dragging) { setDragging(false); void finishRegion({ x: event.clientX, y: event.clientY }); } }} onClick={() => { if (!regionMode) void pick(cursor); }}>
    ...
    <div className="overlay-guide"><span className="overlay-mode-icon">{session.mode === "capture" ? <FrameCorners size={18} /> : session.mode === "measure" ? <Ruler size={18} /> : <Crosshair size={18} />}</span><div><strong>{title}</strong><span>{regionMode ? "Press Escape to cancel" : "Click to sample · Arrow keys move precisely · Enter confirms"}</span></div>{first ? <span className="first-pick"><i style={{ backgroundColor: first.hex }} />{first.hex}<ArrowRight size={13} /></span> : null}<button aria-label="Cancel" onClick={(event) => { event.stopPropagation(); void desktop.invoke("overlay:cancel"); }}><X size={17} /></button></div>
