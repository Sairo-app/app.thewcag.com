...
const UTILITIES: { id: WorkspaceUtility; label: string; title: string; icon: IconType }[] = [
  ...
  { id: "screenshot", label: "Screenshot", title: "Independent screenshot tool", icon: Camera },
...
function normalizeRoute(tool: WorkspaceTool): Route {
  if (tool === "capture" || tool === "captures") return "screenshot";
...
export function Workspace({ platform }: { platform: PlatformInfo }) {
  ...
  function navigate(route: Route) {
    if (!activeAudit && (route === "evidence" || route === "screenshot" || route === "captures")) {
  ...
  return (
    ...
    >
      ...
      <div className="workspace-column" data-standalone={standalone}>
        ...
        <main id="audit-workspace-main" tabIndex={-1} className={`workstage ${showInspector ? "with-inspector" : ""}`}>
