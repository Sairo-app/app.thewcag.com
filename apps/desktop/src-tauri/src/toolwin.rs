...
pub fn open_tool_window(app: AppHandle, kind: String) -> Result<(), String> {
    let (title, w, h) = match kind.as_str() {
        "findings" => ("Findings Register — Accessibility.build", 940.0, 660.0),
        "checklist" => ("WCAG 2.2 Checklist — Accessibility.build", 760.0, 740.0),
        "palette" => ("Palette Contrast — Accessibility.build", 760.0, 660.0),
