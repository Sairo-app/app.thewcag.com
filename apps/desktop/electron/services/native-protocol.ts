...
async function saveFinding(
  ...
  const finding = findingFromDraft(scopedEvidence, draft);
  ...
  if (existing.some((item) => item.key === finding.key)) return finding.key;
  ...
  try {
    ...
    await services.store.remove(evidenceKey).catch(() => undefined);
    throw error;
  }
  await services.store.set(activityKey, [
      {
        ...
        auditId,
        kind: "finding",
        title: "Browser evidence finding saved",
