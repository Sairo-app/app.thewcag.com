...
export default async function BrandPage() {
  ...
  const [[row], entitlements] = await Promise.all([
    db.select({ name: users.brandName, color: users.brandColor, key: users.brandLogoKey }).from(users).where(eq(users.id, userId)).limit(1),
    resolveEntitlements(userId),
  ]);

  const logoUrl = row?.key ? `/api/brand/${userId}/logo` : null;
      ...
      <main id="main" className="app-page mx-auto max-w-2xl px-6 py-10">
        ...
        {entitlements.features.whiteLabelReports ? (
          ...
          <div className="mt-7 rounded-xl border border-border bg-card p-5">
            ...
            <p className="mt-2 text-sm text-muted">Local reports and exports remain free. Pro applies your organization&apos;s identity to hosted client links.</p>
            <Link href="/pricing" className="mt-4 inline-flex rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">View Pro pricing</Link>
          </div>
        )}
      </main>
      <Footer />
