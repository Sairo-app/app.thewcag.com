...
export default async function AdminReports({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (
    <section aria-label="All reports">
      ...
      <ul className="mt-4 space-y-3">
        {rows.map((r) => (
          <li key={r.slug} className="flex items-center gap-4 rounded-xl border border-border bg-card p-3">
            ...
            {r.availabilityStatus !== "disabled" ? <img src={`/api/s/${r.slug}/image`} alt="" loading="lazy" className="h-14 w-20 shrink-0 rounded border border-border object-cover" /> : <div aria-hidden="true" className="flex h-14 w-20 shrink-0 items-center justify-center rounded border border-border bg-background text-xs text-muted">Off</div>}
            <div className="min-w-0 flex-1">
              {r.availabilityStatus !== "disabled" ? <Link href={`/s/${r.slug}`} className="block truncate text-sm font-medium hover:underline">{r.title}</Link> : <span className="block truncate text-sm font-medium">{r.title}</span>}
      ...
      {pages > 1 ? <nav aria-label="Report pages" className="mt-6 flex items-center justify-between text-sm"><span>Page {Math.min(page, pages)} of {pages}</span><span className="flex gap-2">{page > 1 ? <Link href={`/admin/reports?page=${page - 1}`} className="rounded-lg border border-border px-3 py-2">Previous</Link> : null}{page < pages ? <Link href={`/admin/reports?page=${page + 1}`} className="rounded-lg border border-border px-3 py-2">Next</Link> : null}</span></nav> : null}
    </section>
  );
}
